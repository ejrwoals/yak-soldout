...
- 🔗 **주문서 약품 소급 연결**: 마스터에 없던 시절 자유입력으로 적었던 주문 약품을, 마스터 업데이트 후 새 마스터와 fuzzy 매칭해 약품 마스터 관리 화면의 확인 모달에서 마스터 공식명으로 일괄 연결할 수 있습니다.
...
│   ├── order_reconcile.py     # 주문서 ↔ 마스터 소급 연결 — 마스터에 없던 자유입력 주문 약품을 새 마스터로 fuzzy 매칭·연결
...
### 주문서 ↔ 마스터 소급 연결 (Order Reconcile)

주문서는 마스터에 없는 신약을 자유입력으로 먼저 작성할 수 있으므로, 이후 마스터에 그 약이 추가되어도 이미 저장된 주문 항목은 자유입력 이름 그대로 남습니다. 이 고아(orphan) 주문 약품을 새 마스터로 소급 연결하는 기능입니다(`utils/order_reconcile.py`).

- **고아 탐지**: `order_items.drug_name` 중 마스터에 정확히 일치하는 이름이 없는 항목을 같은 이름끼리 묶어 1건으로 본 뒤, 매처의 `drug_matcher.top_candidates`로 새 마스터와 fuzzy 매칭해 고아마다 점수순 상위 후보 여러 개를 만듭니다. 최상위 점수가 기준 미만이거나 표시할 후보가 없으면 그 고아는 아예 제외합니다.
- **확인 모달(다중 후보 드롭다운)**: 엑셀 업로드(마스터 갱신) 직후 약품 마스터 관리 화면이 `GET /api/order-ocr/link-candidates`로 후보를 받아 확인 모달을 띄웁니다. 고아마다 `<select>` 드롭다운으로 여러 후보(`공식명 (점수%)`)를 제시하며 기본값은 "연결 안 함"입니다. 최상위 후보가 고확신(≥`HIGH_SCORE`)인 경우에만 그 후보를 자동 선택하고, 그 외에는 오매칭을 막기 위해 "연결 안 함"으로 둡니다. 사용자가 고른 항목만 `POST /api/order-ocr/link`로 적용되어 해당 고아 이름의 모든 `order_items.drug_name`을 마스터 공식명으로 일괄 갱신합니다.
- **멱등성**: 연결된 항목은 이후 마스터와 정확히 일치하므로 다시 후보로 제시되지 않습니다.
- **주문 자유입력 약품 노출**: 약품 마스터 관리 현황 카드는 마스터에 없는 자유입력 주문 약품이 있으면 등록 개수 옆에 "주문 자유입력 N" 칩을 표시합니다(`drug_master.status()`의 `orphan_order_count`, `db.count_orphan_order_drugs`). 칩을 누르면 `GET /api/drug-master/orphan-drugs`로 각 고아 약품명·주문 항목 수·마지막 주문일자를 받아 목록 모달로 보여줘, 어떤 약을 마스터에 추가하면 연결되는지 가늠할 수 있습니다(`db.list_orphan_order_drugs`).
...
- `GET /api/order-ocr/link-candidates` - 마스터 갱신 후 마스터에 없던 자유입력 주문 약품의 소급 연결 후보 조회
- `POST /api/order-ocr/link` - 선택한 고아 주문 약품명을 마스터 공식명으로 일괄 연결
- `GET /drug-master` - 약품 마스터 등록 화면
- `GET /api/drug-master` - 약품 마스터 등록 현황 조회 (포장단위 수집 현황 + 주문 자유입력(고아) 약품 수 포함)
- `GET /api/drug-master/orphan-drugs` - 주문서 자유입력 약품(마스터 미일치) 목록 (이름·주문 항목 수·마지막 주문일자)
