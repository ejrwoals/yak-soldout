...
def execute_search_sync(app_state, progress_queue=None):
    ...
    try:
        ...
        # 지오팜 검색 (활성화된 경우) - 사이클 조기 종료 체크
        geopharm_active = config_file.get('지오팜활성화', 'false').lower() == 'true'
        if app_state.cycle_terminated:
            log_message("🏥 긴급 재고 발견으로 지오팜 검색 건너 뜀")
        elif geopharm_active and app_state.config.has_geopharm_credentials():
            log_message("🏥 지오팜 검색 시작...")
            # 지오영에서 수집한 보험코드 사용
            if hasattr(all_drugs, '__iter__') and len(all_drugs) > 0:
                insurance_codes = {}
                for drug in all_drugs:
                ...
                if insurance_codes:
                    geopharm_drugs, geopharm_errors = search_geopharm_sync(app_state, insurance_codes, excluded_by_distributor.get("지오팜", []), progress_queue, urgent_drugs)
                    all_drugs.extend(geopharm_drugs)
                    errors.extend(geopharm_errors)
                else:
                    log_message("⚠️ 지오영에서 보험코드를 수집하지 못했습니다")
            else:
                log_message("⚠️ 지오영 검색 결과가 없어 지오팜 검색을 건너뜁니다")
...
def search_geopharm_sync(app_state, insurance_codes: Dict[str, str], excluded_names: List[str], progress_queue=None, urgent_drugs=None) -> tuple:
    """지오팜 검색 (동기)"""
    ...
    try:
        ...
        scraper = GeoPharmScraper()
        page = browser_mgr.new_page()

        # 로그인
        log_message("🤖 지오팜에 로그인하는 중입니다...")
        region = app_state.config.geopharm_region or '01'
        if not scraper.login(page, app_state.config.geopharm_id, app_state.config.geopharm_password, region):
            raise Exception("지오팜 로그인 실패")

        log_message("✓ 지오팜 로그인 성공")
        ...
        for i, (insurance_code, original_name) in enumerate(insurance_codes.items(), 1):
            ...
            try:
                drugs = scraper._search_by_insurance_code(insurance_code, original_name)
                for drug in drugs:
                    # 지오팜 exclusion 체크: 규격 정보까지 포함한 전체 이름으로 매칭
                    unit_display = f" [{drug.unit}]" if drug.unit else ""
                    full_name = f"{drug.name}{unit_display}"
                    drug.is_excluded_from_alert = full_name in excluded_names
                    drug.search_insurance_code = insurance_code
                    drug.original_drug_name = original_name
                    drug.distributor = "지오팜"

                # 검색 결과 로그
                if drugs:
                    log_message(f"🔍 지오팜 검색 완료 ({i}/{len(insurance_codes)}): {original_name} ({insurance_code}) - {len(drugs)}개 규격 발견")
                    ...
                    for drug in drugs:
                        ...
                        drug_data = {
                            ...
                            "main_stock": main_stock,
                            "incheon_stock": "-",
                            "company": drug.company if hasattr(drug, 'company') else "지오팜",
                            "distributor": "지오팜",
                            "has_stock": has_stock,
                            "unit": drug.unit
                        }
                    ...
                    if urgent_stock_found:
                        ...
                        urgent_alert_msg = {
                            "type": "urgent_alert",
                            "drug": {
                                "name": f"지오팜 재고 발견: {specs_display}",
                    ...
                    log_message(f"❌ 지오팜 검색 실패 ({i}/{len(insurance_codes)}): {original_name} ({insurance_code}) - 검색 결과 없음")
                    errors.append(f"{original_name} ({insurance_code}): 검색 결과 없음")
