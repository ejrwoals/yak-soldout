#!/usr/bin/env python3
"""손글씨 주문지 OCR 서비스 (Cloud Run 웹 UI 전용, 자체 완결).

기존 utils/ocr_service.py 와 동일한 Gemini 호출 로직을 스택 1에 자체 포함시킨 사본.
(두 스택은 서로 코드를 import하지 않으므로 필요한 로직만 복제한다.)
배포 시 GEMINI_API_KEY 는 Cloud Run Secret 으로 주입한다.
"""
...
# cloud_web/.env 로드 (Cloud Run에서는 환경변수로 직접 주입되므로 파일이 없어도 무해)
...
def extract_order_items(image_bytes: bytes, mime_type: str) -> list[dict]:
    ...
    if not api_key:
        raise OcrConfigError(
            "GEMINI_API_KEY 가 설정되지 않았습니다. cloud_web/.env 를 확인하세요 (.env.example 참고)."
        )
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "`uv pip install -r cloud_web/requirements.txt` 를 실행하세요."
        ) from e
