...
def save_reviewed_order(client, pharmacy_id: str, order_date: str, order_round: int,
                        ...
                        image_mime: str | None = None, overwrite: bool = False) -> str:
    """주문 저장 후 order_id 반환. 중복이면 DuplicateOrderError (overwrite=True 면 기존 주문 교체)."""
    ...
    if dup.data:
        ...
        # 덮어쓰기: 기존 주문 삭제 (order_items 는 FK CASCADE) + 원본 이미지 정리
        for old in dup.data:
            client.table("orders").delete().eq("id", old["id"]).execute()
            if old.get("image_path"):
                try:
                    client.storage.from_("order-images").remove([old["image_path"]])
                except Exception:
                    pass  # 이미지 정리 실패는 저장을 막지 않는다
