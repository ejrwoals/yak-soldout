...
async def api_orders(authorization: str = Header(None)):
    ...
    res = (
        client.table("orders")
        .select("id, order_date, order_round, status, image_path, created_at, order_items(*)")
    ...
    for o in orders:
        ...
        o["order_items"] = items
        o["has_image"] = bool(o.get("image_path"))
        o.pop("image_path", None)  # Storage 경로는 노출하지 않는다 (이미지는 전용 엔드포인트로)
...
async def api_order_image(order_id: str, authorization: str = Header(None)):
    """주문의 원본 주문지 이미지 반환. RLS(사용자 토큰)로 소속 확인 후 Storage 에서 내려준다."""
    client = _user_client(authorization)
    _require_membership(_token_sub(authorization))
    res = client.table("orders").select("image_path").eq("id", order_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="주문을 찾을 수 없습니다.")
    path = res.data[0].get("image_path")
    if not path:
        raise HTTPException(status_code=404, detail="이 주문에는 원본 이미지가 없습니다.")
    try:
        data = await asyncio.to_thread(
            lambda: _service_client().storage.from_("order-images").download(path)
        )
    except Exception:
        raise HTTPException(status_code=404, detail="이미지 파일을 불러올 수 없습니다.")
    mime = _IMG_EXT_MIME.get(os.path.splitext(path)[1].lower(), "image/jpeg")
    return Response(content=data, media_type=mime)


@app.delete("/api/orders/{order_id}")
async def api_order_delete(order_id: str, authorization: str = Header(None)):
    """주문 1건 삭제 (품목은 FK CASCADE, 원본 이미지도 함께 정리). 소속 멤버면 가능."""
    client = _user_client(authorization)
    _require_membership(_token_sub(authorization))
    res = client.table("orders").select("id, image_path").eq("id", order_id).execute()
    if not res.data:
        raise HTTPException(status_code=404, detail="주문을 찾을 수 없습니다.")
    image_path = res.data[0].get("image_path")
    client.table("orders").delete().eq("id", order_id).execute()
    if image_path:
        try:
            _service_client().storage.from_("order-images").remove([image_path])
        except Exception:
            pass  # 이미지 정리 실패는 삭제 성공에 영향 없음
    return {"deleted": True}
...
async def api_save(
    ...
    try:
        order_id = await asyncio.to_thread(
            ...
            client, membership["pharmacy_id"], order_date, order_round, items,
            image_bytes, image_mime, overwrite,
        )
    ...
    # 자유입력(마스터에 없는) 약품을 마스터에 자동 등록 → 이후 OCR 매칭·자동완성에 활용.
    # 보강 단계이므로 실패해도 주문 저장 자체는 성공으로 처리한다.
    registered = 0
    try:
        result = await asyncio.to_thread(
            master_repo.register_free_input_drugs, client, membership["pharmacy_id"], items
        )
        registered = result.get("added", 0)
        if registered:
            _index_cache.pop(membership["pharmacy_id"], None)  # 매칭 인덱스에 즉시 반영
    except Exception:
        pass

    return {"saved": True, "order_id": order_id, "registered_drugs": registered}
